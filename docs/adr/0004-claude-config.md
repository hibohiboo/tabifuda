...
状態: 採用 / 日付: 2026-07-13(2026-07-19更新: agents/ を追加。2026-09-01更新: rules/ を追加)
...
### ルール置き場: `.claude/rules/`(2026-09-01追加)

CLAUDE.mdは全プロジェクト共通のルールに絞り、レイヤー/ディレクトリ固有の
実装規約(CLAUDE.mdの「Rust規約」節に相当するTS/APIレイヤー版など)は
`.claude/rules/*.md` に分割する。ファイルは `paths:` frontmatterで
対象globを指定でき、該当パスのファイルをClaudeが読むときのみ
コンテキストに載る(CLAUDE.mdの肥大化を防ぐ)。方針:

- ルールは「Claudeへのガイダンス」であり強制ではない。強制が要る操作は
  引き続き permissions/hooks で扱う
- 規約内容の正が docs/design/ 側にある場合は複製せず「正は〜」参照に
  留める(SSoT原則、docs/adr/0007-ssot-single-responsibility.md)。
  CLAUDE.md本体に既にある規定(1ファイル1責務等)も同様に参照のみとする
- 既知の制約(2026-09-01時点): `paths:` のYAMLリスト形式が環境により
  効かないことがあるとの報告がある(upstream issue #17204)。
  導入時・追加時に動作を確認する
