...
### フォーク出力(セッション終了後のシナリオ保存。CLIの決定)

「シナリオパッチ(構造化)」の設計意図**フォーク公開**(改編シナリオ=
元シナリオ+パッチ列)の最初の実装。MVPでは「元+パッチ列」形式の
永続フォーマットは定義せず、**パッチ適用済みスナップショットの書き出し**で
代替する(パッチ列そのものは冒険記のイベントログに残っており、由来は
失われない。パッチ列形式での公開・作者への還流は将来要望メモ§6)。

- セッションが`Ended`に達し、かつ`ScenarioPatched`が1回以上あるとき、
  CLIは「フォークとして保存するか」を尋ねる
- 出力は`Session.scenario`(パッチ適用済み)を基に次の変換を加えたもの:
  - `meta.id`: 出力ファイル名の語幹と揃える(下記。id空間の全体を
    CLIは知り得ないため、ファイル衝突回避と同じ発番を共有する)
  - `meta.forked_from`: 元シナリオのid
  - `PatchOp::DealCard`で配られたカードを、**配布時に居たシーンの`deals`へ
    追加**する(イベントログの`SceneEntered`で現在シーンを追跡して復元。
    同一シーン×同一カード×同一宛先の重複は1件にまとめる)。
    `DealCard`は「その場で配る」でありシーンの入場時配布に残らないため、
    この変換をしないとフォークを次に遊んでも配られない
- 出力先は元ファイルと同じディレクトリの`{元ファイル名}-fork.json`
  (既存ファイルと衝突する場合は連番を付す)
...
- CLIは複数キャラを同時に操作するUIを持たないため、**パーティ先頭
  ...
  同じ規則が成立する)
- 終了時のマスターへの書き戻し(将来要望メモ§3の採用)は別サイクルで扱う
