...
**進捗(2026-07-18)**: 仕様判断を伴わない安全修正(M1・M2・L1・H3)は適用済み
(各項目に「✅ 対応済み」を付記)。人間の判断が要る項目(H1・H2・M3・L2)は
末尾「判断タスクリスト」の順で一つずつ処理する。
...
### H1. `Target::Character(CharacterId)` はシナリオデータ内で表現不能
...
### H2. ロールの真実の源が二重(`Actor.role` vs `Session.roles`)
...
### L2. 型レベルの文字列長上限(cross-cutting P1 項目)が未着手
...
④本ファイルに『対応済み』追記」の順で進める。1項目ずつ区切って合意を取る。
