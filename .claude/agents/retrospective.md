...
2. docs/tasks/projects/phaseN/task.md の完了条件と各サイクルの要求
...
0. **冒頭の位置づけ宣言**は定型文を使う(区分の定義を個別に再掲しない):
   「位置づけ: 記録(docs/README.md『文書区分の定義』)。ここで挙げた
   対応項目は反映先の文書が正であり、本文書は経緯を残す。」
1. **成果**: 完了条件の充足状況とサイクル別の成果物の表
2. **うまくいったこと**: 再現したいプラクティス(根拠となる実例つき)
3. **課題**: ジャーナル記録と対応状況(記録済みなら日付で参照)
4. **気づきと対応(反映先)**: 気づき/対応案/反映先候補の表。
   反映先候補は既存文書(future-requirements.md、次の projects/phaseN/task.md、
